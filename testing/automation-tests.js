  ...
  try {
    ...
    for (const viewport of viewports) {
      try {
        ...
        await page.screenshot({
          path: path.join(
            SCREENSHOTS_DIR,
            `test6-${viewport.name.toLowerCase().replace(' ', '-')}.png`
          ),
        })
      } catch (error) {
    ...
    try {
      ...
      testResults.addTest(
        ...
        lazyImages.length > 0 ? 'passed' : 'warning',
        {
      ...
      if (footer) {
        const footerContent = await page.evaluate(() => {
          ...
          return {
            ...
            hasSocialLinks: !!footer.querySelector(
              '[href*="facebook"], [href*="twitter"], [href*="instagram"], [href*="linkedin"]'
            ),

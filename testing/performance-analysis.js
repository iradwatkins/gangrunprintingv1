...
async function analyzePerformance() {
  ...
  try {
    ...
    const report = {
      ...
      resources: {
        images: {
          ...
          avgLoadTime:
            resourceTimings.images.reduce((a, b) => a + b.duration, 0) /
            (resourceTimings.images.length || 1),
        },
        scripts: {
          count: resourceTimings.scripts.length,
          totalSize: resourceTimings.scripts.reduce((a, b) => a + b.transferSize, 0),
          avgLoadTime:
            resourceTimings.scripts.reduce((a, b) => a + b.duration, 0) /
            (resourceTimings.scripts.length || 1),
        },
        stylesheets: {
          count: resourceTimings.stylesheets.length,
          totalSize: resourceTimings.stylesheets.reduce((a, b) => a + b.transferSize, 0),
          avgLoadTime:
            resourceTimings.stylesheets.reduce((a, b) => a + b.duration, 0) /
            (resourceTimings.stylesheets.length || 1),
        },
      },
    ...
    console.log(
      `   LCP (Largest Contentful Paint): ${webVitals.lcp.toFixed(2)}ms [${report.coreWebVitals.lcp.rating}]`
    )
    console.log(
      `   FID (First Input Delay): ${webVitals.fid.toFixed(2)}ms [${report.coreWebVitals.fid.rating}]`
    )
    console.log(
      `   CLS (Cumulative Layout Shift): ${webVitals.cls.toFixed(3)} [${report.coreWebVitals.cls.rating}]`
    )
    ...
    console.log('\n📦 Resource Analysis:')
    console.log(
      `   Images: ${report.resources.images.count} files, ${(report.resources.images.totalSize / 1024).toFixed(2)} KB`
    )
    console.log(
      `   Scripts: ${report.resources.scripts.count} files, ${(report.resources.scripts.totalSize / 1024).toFixed(2)} KB`
    )
    console.log(
      `   Stylesheets: ${report.resources.stylesheets.count} files, ${(report.resources.stylesheets.totalSize / 1024).toFixed(2)} KB`
    )
    ...
    if (webVitals.lcp > 2500) {
      recommendations.push(
        '• Optimize Largest Contentful Paint - consider preloading critical resources'
      )
    }

...
class OrderFlowTestRunner {
  ...
  async navigateToProduct(page, result) {
    ...
    try {
      ...
      const screenshot = await this.takeScreenshot(page, `iteration-${result.iteration}-product-page`);
  ...
  async addProductToCart(page, result) {
    ...
    try {
      ...
      const addToCartButton = await page.$('button:has-text("Add to Cart"), button:has-text("ADD TO CART")');
      
      if (!addToCartButton) {
        // Try alternative selectors
        await page.click('[data-testid="add-to-cart-button"], .add-to-cart-btn, button[type="submit"]');
      } else {
      ...
      const screenshot = await this.takeScreenshot(page, `iteration-${result.iteration}-added-to-cart`);
      result.screenshots.push(screenshot);
  ...
  async proceedToCheckout(page, result) {
    ...
    try {
      ...
      await page.click('button:has-text("Proceed to Checkout"), a[href="/checkout"], button:has-text("Checkout")');
      
      // Wait for checkout page to load
      await page.waitForSelector('input[name="email"], #email', { timeout: 15000 });
      
      const screenshot = await this.takeScreenshot(page, `iteration-${result.iteration}-checkout-page`);
      result.screenshots.push(screenshot);
  ...
  async fillCustomerInformation(page, result) {
    ...
    try {
      ...
      const screenshot = await this.takeScreenshot(page, `iteration-${result.iteration}-customer-info`);
      result.screenshots.push(screenshot);
      ...
      if (testCashOption) {
        ...
        await page.evaluate(() => {
          ...
          for (const label of labels) {
            if (label.textContent.toLowerCase().includes('test') || 
  ...
  async completeOrder(page, result) {
    ...
    try {
      ...
      await page.click('button:has-text("Place Order"), button:has-text("Complete Order"), button:has-text("Submit Order")');
      
      // Wait for order processing
      await page.waitForTimeout(5000);
      
      // Check for success page or confirmation
      const successIndicator = await page.$('.order-success, .confirmation, [data-testid="order-confirmation"]');
      ...
      const screenshot = await this.takeScreenshot(page, `iteration-${result.iteration}-order-complete`);
      result.screenshots.push(screenshot);
  ...
  async verifyOrderConfirmation(page, result) {
    ...
    try {
      ...
      const screenshot = await this.takeScreenshot(page, `iteration-${result.iteration}-confirmation`);
      result.screenshots.push(screenshot);
  ...
  async verifyAdminVisibility(result) {
    ...
    try {
      ...
      if (result.orderDetails && result.orderDetails.orderNumber) {
        const response = await fetch(`${TEST_CONFIG.baseUrl}/api/orders/check/${result.orderDetails.orderNumber}`);

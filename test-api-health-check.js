...
async function runHealthCheck() {
  ...
  const avgResponseTime = results
    ...
    .reduce((acc, r) => acc + r.responseTime, 0) / results.filter(r => r.responseTime).length;
  ...
  const criticalIssues = results.filter(r => 
    criticalEndpoints.includes(r.endpoint) && r.status !== 'PASS'
  );

...
class CheckoutTester {
  ...
  setupErrorMonitoring() {
    ...
    this.page.on('console', (msg) => {
      ...
      if (type === 'error') {
        ...
        (type === 'warning' && text.includes('CSP')) ||
  ...
  async testProductPage() {
    ...
    try {
      ...
      const hasAddToCart = await this.page.$(
        'button:has-text("Add to Cart"), button[type="submit"]'
      )

      if (!hasTitle) {
        await this.recordStep('Navigate to Product Page', 'failure', {
          reason: 'No product title found',
        })
        return false
      }
  ...
  async testPaymentPage() {
    ...
    try {
      ...
      const cashAppErrors = this.errors.filter(
        (e) => e.message && e.message.toLowerCase().includes('cash app')
      )
  ...
  generateReport() {
    const report = {
      testRun: {
        ...
        duration:
          this.steps.length > 0
            ? new Date(this.steps[this.steps.length - 1].timestamp) -
              new Date(this.steps[0].timestamp)
            : 0,
      },
      summary: {
  ...
  printSummary(report) {
    ...
    console.log(
      `✅ Successful Steps: ${report.summary.successfulSteps}/${report.summary.totalSteps}`
    )

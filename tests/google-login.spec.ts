...
test.describe('Google OAuth Login Tests', () => {
  ...
  test('Test 1: Google OAuth button exists and is clickable', async ({ page }) => {
    ...
    const googleButton = page.locator('button:has-text("Sign in with Google"), a:has-text("Sign in with Google"), [aria-label*="Google"]');
    ...
    const hasError = await page.locator('text=/configuration error/i').isVisible().catch(() => false);
    ...
    if (hasError) {
      ...
      const errorMessage = await page.locator('[class*="error"], [id*="error"]').textContent().catch(() => 'No error message found');
  ...
  test('Test 3: Test OAuth flow with mock credentials', async ({ page }) => {
    ...
    if (await googleButton.isVisible()) {
      ...
      if (currentUrl.includes('/auth/error')) {
        // Get error details
        const errorCode = await page.locator('text=/error code/i').textContent().catch(() => '');
        const errorMessage = await page.locator('text=/error|problem/i').textContent().catch(() => '');

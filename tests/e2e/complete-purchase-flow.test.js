...
describe('Complete E-Commerce Purchase Flow', () => {
  ...
  describe('Phase 1: Product Configuration & Cart', () => {
    test('Should navigate to products and select Premium Business Cards', async () => {
      ...
      const premiumCardSelector =
        '[data-product-slug="premium-business-cards"], [href*="premium-business-cards"]'
    ...
    test('Should configure product with all add-on positions', async () => {
      ...
      await page.waitForSelector('.product-options, [data-testid="product-form"]', {
        timeout: 10000,
      })
      ...
      const rushProductionSelector =
        '[data-position="ABOVE_DROPDOWN"], [data-addon="rush-production"]'
      ...
      // Rounded Corners
      const roundedCornersSelector =
        '[data-position="BELOW_DROPDOWN"][data-addon="rounded-corners"], [data-addon="rounded-corners"]'
    ...
    test('Should add configured product to cart', async () => {
      ...
      const addToCartSelector =
        'button:contains("Add to Cart"), [data-testid="add-to-cart"], button[type="submit"]'
      ...
      if (addToCartButton) {
        ...
        try {
          await page.waitForSelector('.cart-notification, [data-testid="cart-success"]', {
            timeout: 5000,
          })
  ...
  describe('Phase 2: Customer Information & Shipping', () => {
    ...
    test('Should select shipping method', async () => {
      ...
      try {
        ...
        const firstShippingOption = await page.$(
          'input[name="shipping"], .shipping-option input[type="radio"]'
        )
  ...
  describe('Phase 3: Payment Processing', () => {
    test('Should process Square payment with test card', async () => {
      ...
      await page.waitForSelector('.payment-form, [data-testid="payment-section"]', {
        timeout: 15000,
      })
      ...
      try {
        ...
        let squareFrame = frames.find(
          (frame) => frame.url().includes('squareup.com') || frame.url().includes('square')
        )
        ...
        if (squareFrame) {
          ...
          // Fill expiry
          await squareFrame.fill(
            'input[name="expirationDate"], #expiration-date',
            `${TEST_CARD.expMonth}${TEST_CARD.expYear.slice(-2)}`
          )
        ...
        const submitButton = await page.$(
          'button:has-text("Place Order"), button:has-text("Pay Now"), [data-testid="submit-payment"]'
        )
        if (submitButton) {
          ...
          try {
            await page.waitForSelector(
              '.order-confirmation, [data-testid="order-success"], h1:has-text("Success")',
              { timeout: 30000 }
            )
            ...
            const orderNumber = await page
              .$eval('.order-number, [data-testid="order-number"]', (el) => el.textContent)
              .catch(() => 'Not found')
            const orderTotal = await page
              .$eval('.order-total, [data-testid="order-total"]', (el) => el.textContent)
              .catch(() => 'Not found')
            ...
            if (currentUrl.includes('success') || currentUrl.includes('confirmation')) {
              ...
              const errorMessage = await page
                .$eval('.error-message, .alert-error', (el) => el.textContent)
                .catch(() => null)
              if (errorMessage) {

...
class FileApprovalTestHelpers {
  ...
  async checkDatabaseState(orderId, expectedState) {
    ...
    const response = await this.page.request.get(`${TEST_CONFIG.baseURL}/api/orders/${orderId}/files`);
...
test.describe('File Approval System - Complete Workflow', () => {
  ...
  test('1. Customer uploads artwork files', async () => {
    ...
    await expect(adminPage.locator('[data-testid="artwork-file-list"]')).toContainText('test-business-card.pdf');
    await expect(adminPage.locator('[data-testid="artwork-file-list"]')).toContainText('test-logo.jpg');
  ...
  test('2. Admin uploads proof for customer approval', async () => {
    ...
    await expect(customerPage.locator('[data-testid="waiting-proofs"]')).toContainText('Awaiting Your Approval');
  ...
  test('3. Customer rejects proof and requests changes', async () => {
    ...
    const proofFileId = await customerPage.getAttribute('[data-testid="proof-file-card"]', 'data-file-id');
    ...
    await expect(customerPage.locator('[data-testid="rejected-proofs"]')).toContainText('Changes Requested');
  ...
  test('5. Customer approves revised proof', async () => {
    ...
    const waitingProofs = await customerPage.locator('[data-testid="waiting-proofs"] [data-testid="proof-file-card"]');
    ...
    // Verify database state
    const dbState = await adminHelpers.checkDatabaseState(testOrderId);
    const approvedProof = dbState.files.find(f => 
      f.fileType === 'ADMIN_PROOF' && f.approvalStatus === 'APPROVED'
    );
  ...
  test('6. Order status updates to ready for production', async () => {
    ...
    await adminPage.goto(`${TEST_CONFIG.baseURL}/admin/orders/${testOrderId}`);
    await expect(adminPage.locator('[data-testid="order-status"]')).toContainText('Ready for Production');

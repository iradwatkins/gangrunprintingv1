...
describe('Paper Stock Module Tests', () => {
  ...
  describe('Test 2: Coating Cascade Logic', () => {
    test('should update coating options when paper changes', async () => {
      ...
      const cascadeTestPapers = ['14pt-cardstock', '16pt-cardstock', '100lb-text']
      ...
      for (const paperType of cascadeTestPapers) {
        ...
        try {
          ...
          if (paperOption) {
            ...
            if (allPaperOptions.length > 0) {
              await allPaperOptions[
                Math.min(cascadeTestPapers.indexOf(paperType), allPaperOptions.length - 1)
              ].click()
            }
          }
        } catch (error) {
        ...
        const coatingDropdownExists = await helpers.waitForSelectorSafe(
          selectors.paperStock.coatingDropdown,
          5000
        )
  ...
  describe('Test 3: Sides Selection & Pricing', () => {
    test('should handle sides selection and pricing correctly', async () => {
      ...
      // Test sides options
      const sidesDropdownExists = await helpers.waitForSelectorSafe(
        selectors.paperStock.sidesDropdown,
        5000
      )
        ...
        if (singleSidedOption || sidesOptions.length > 0) {
          ...
          if (doubleSidedOption) {
            ...
            if (singleSidedPrice && doubleSidedPrice) {
              ...
              console.log(
                `Double-sided premium: $${priceDifference.toFixed(2)} (${percentIncrease.toFixed(1)}%)`
              )
      ...
      for (let i = 0; i < testPaperCount; i++) {
        ...
        const sidesExists = await helpers.waitForSelectorSafe(
          selectors.paperStock.sidesDropdown,
          3000
        )
  ...
  describe('Test 4: Paper Stock Cascade Dependencies', () => {
    test('should handle complete cascade updates correctly', async () => {
      ...
      if (paperOptions.length > 0) {
        ...
        const coatingExists = await helpers.waitForSelectorSafe(
          selectors.paperStock.coatingDropdown,
          3000
        )
        if (coatingExists) {
      ...
      if (overridePaperOptions.length > 0) {
        ...
        const overrideCoatingExists = await helpers.waitForSelectorSafe(
          selectors.paperStock.coatingDropdown,
          3000
        )
  ...
  describe('Test 5: Paper Stock Customer Display', () => {
    test('should display paper options correctly on customer frontend', async () => {
      ...
      try {
        ...
        const paperSelectorExists = await helpers.waitForSelectorSafe(
          selectors.customer.paperSelector,
          10000
        )
        ...
        const customerCoatingExists = await helpers.waitForSelectorSafe(
          '[data-testid="customer-coating"]',
          5000
        )
        ...
        const customerSidesExists = await helpers.waitForSelectorSafe(
          '[data-testid="customer-sides"]',
          5000
        )

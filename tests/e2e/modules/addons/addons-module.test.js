...
describe('Addons Module Tests', () => {
          ...
          for (const addonId of config.addons) {
            ...
            if (!addonSelected) {
              ...
              for (const checkbox of checkboxes) {
                try {
                  const parent = await checkbox.evaluateHandle(el => el.closest('label, .addon-item, .addon-option'));
                  if (parent) {
        ...
        for (const result of addonResults) {
          if (result.price && basePrice) {
            ...
            if (result.priceIncrease > 0) {
              console.log(`✅ ${result.description} adds $${result.priceIncrease.toFixed(2)} to price`);
            }
          }
        }

      } else {
        console.log('⚠️ Addons module not found - may be no addons configured for this product type');
      }
  ...
  describe('Test 2: Variable Data Configuration', () => {
    test('should configure variable data addon correctly', async () => {
      ...
      const variableDataExists = await helpers.waitForSelectorSafe(selectors.addons.variableData.enable, 5000);
        ...
        if (locationsInput) {
          ...
          for (let i = 1; i < locationPrices.length; i++) {
            ...
            if (current.price && previous.price && current.locations > previous.locations) {
              ...
              console.log(`✅ Price increases with more locations: ${previous.locations} -> ${current.locations}`);
            }
          }
        }
  ...
  describe('Test 3: Perforation & Special Addons', () => {
    test('should configure perforation and special addons correctly', async () => {
      ...
      const perforationExists = await helpers.waitForSelectorSafe(selectors.addons.perforation.enable, 5000);
      ...
      const cornerRoundingExists = await helpers.waitForSelectorSafe(selectors.addons.cornerRounding.enable, 5000);
    ...
    test('should handle addon dependencies and restrictions correctly', async () => {
      ...
      for (const pricingTest of pricingModelTests) {
        ...
        if (variableDataForPricing) {
          ...
          console.log(`Qty ${pricingTest.quantity}: Addon cost $${addonCost.toFixed(2)}, Per unit $${costPerUnit.toFixed(4)}`);
      ...
      if (addonPricingResults.length > 1) {
        const costPerUnitVariance = Math.max(...addonPricingResults.map(r => r.costPerUnit)) -
  ...
  describe('Test 5: Addon Frontend Integration', () => {
    test('should display and function correctly on customer frontend', async () => {
      ...
      try {
        ...
        const addonsDisplayExists = await helpers.waitForSelectorSafe(selectors.customer.addonsSelector, 10000);
        ...
        if (addonsDisplayExists) {
          ...
          const customerAddonOptions = await page.$$('[data-testid*="customer-addon"], .addon-option, .addon-item');
          ...
          const customerVariableDataConfig = await page.$('[data-testid="customer-variable-data-config"]');
          ...
          const addonPriceElements = await page.$$('[data-testid*="addon-price"], .addon-price, .price-addon');
          for (const priceElement of addonPriceElements) {
            ...
            if (priceText.includes('$') || priceText.includes('cost') || priceText.includes('add')) {

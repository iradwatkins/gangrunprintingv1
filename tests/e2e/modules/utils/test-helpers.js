...
class TestHelpers {
  ...
  async uploadDemoImage() {
    ...
    try {
      ...
      await this.page.waitForSelector(selectors.admin.imageUpload.uploadZone, { timeout: TIMEOUTS.default });
  ...
  async configurePaperStock(config) {
    ...
    // Select paper type
    await this.page.waitForSelector(selectors.paperStock.paperDropdown, { timeout: TIMEOUTS.default });
  ...
  async configureAddons(config) {
    ...
    if (config.variableData) {
      ...
      if (config.variableData.locations) {
        await this.page.fill(selectors.addons.variableData.locationsInput, config.variableData.locations.toString());
      }
    }
    ...
    if (config.banding) {
      ...
      if (config.banding.itemsPerBundle) {
        await this.page.fill(selectors.addons.banding.itemsPerBundle, config.banding.itemsPerBundle.toString());
      }
    }

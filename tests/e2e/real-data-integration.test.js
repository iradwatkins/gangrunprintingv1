...
class RealDataTestHelper {
  ...
  async fillBasicInfo(productData, realData) {
    ...
    if (realData.categories.length > 0) {
      ...
      if (category) {
        ...
        const categorySelector =
          'select[name="categoryId"], #categoryId, [data-testid="category-select"]'
    ...
    if (productData.description) {
      const descSelector =
        'textarea[name="description"], #description, [data-testid="product-description"]'
    ...
    for (const selector of saveSelectors) {
      try {
        ...
        if (button) {
          ...
          const successSelectors = ['.success-message', '.alert-success', '[data-testid="success"]']
  ...
  async checkForErrors() {
    ...
    const errorSelectors = ['.error', '.alert-error', '.text-red-500', '[data-testid="error"]']

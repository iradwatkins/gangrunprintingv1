...
(async () => {
  ...
  page.on('console', msg => {
    const text = msg.text();
    if (text.includes('error') || text.includes('Error') || text.includes('[Frontend]') || text.includes('[API]')) {
  ...
  try {
    ...
    console.log(`📋 Page Elements: ${formExists.formCount} forms, ${formExists.buttonCount} buttons\n`);
    ...
    if (apiCalls.length > 0) {
      ...
      apiCalls.forEach((call, index) => {
        console.log(`   ${index + 1}. Status: ${call.status}, JSON: ${call.isJSON ? '✅' : '❌'}, Time: ${call.timestamp}`);
      });

...
async function runOrderFlowTest(testRun) {
  ...
  try {
    ...
    try {
      ...
      results.steps.push({ step: 5, name: 'Fill customer information', success: false, error: error.message });
    }
    ...
    try {
      ...
      results.steps.push({ step: 7, name: 'Select shipping and continue to payment', success: true });
    } catch (error) {
    ...
    try {
      ...
      const paymentSelected = await page.evaluate(() => {
        ...
        let cashElement = elements.find(el =>
          el.textContent && el.textContent.includes('Test Cash Payment')
        );
        
        // Strategy 2: Look for "Cash" payment option
        if (!cashElement) {
          cashElement = elements.find(el =>
            el.textContent && (
        ...
        if (!cashElement) {
          ...
          cashElement = radioButtons.find(radio =>
            radio.value.toLowerCase().includes('cash') ||
            radio.id.toLowerCase().includes('cash')
          );
        }
        ...
        if (cashElement) {
          if (cashElement.tagName === 'INPUT') {
            cashElement.click();
            return { success: true, method: 'radio button', text: cashElement.value || 'cash option' };
          } else if (cashElement.tagName === 'LABEL') {
            cashElement.click();
            return { success: true, method: 'label', text: cashElement.textContent.trim() };
          } else {
            // Try to find associated input
            const input = cashElement.querySelector('input[type="radio"]') || 
      ...
      if (paymentSelected.success) {
        ...
        log('⚠️ Could not find Test Cash Payment option, checking all payment options...', 'warning');
      ...
      results.steps.push({ step: 8, name: 'Select Test Cash Payment', success: paymentSelected.success });
    } catch (error) {
      log(`⚠️ Payment selection error: ${error.message}`, 'warning');
      results.steps.push({ step: 8, name: 'Select Test Cash Payment', success: false, error: error.message });
    }
    ...
    try {
      ...
      const orderCompleted = await page.evaluate(() => {
        ...
        if (submitBtn && !submitBtn.disabled) {
          ...
          submitBtn.click();
          return { success: true, buttonText: submitBtn.textContent || submitBtn.value || 'Order button clicked' };
        }
...
async function runCompleteTest() {
  ...
  console.log(`📸 Screenshots saved to: ${path.join(__dirname, 'screenshots', 'order-flow-test-fixed')}`);

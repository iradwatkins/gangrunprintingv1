...
async function testNetworkRequest(url, options = {}) {
  ...
  try {
    ...
    console.log(
      `${statusColor}${status} ${colors.reset}${options.method || 'GET'} ${url.replace(BASE_URL, '')} ${colors.cyan}${duration}ms ${colors.reset}${size} bytes`
    )
...
${colors.reset}`)
  ...
  await runTest('TEST 5: Addon Display Positioning (ABOVE/IN/BELOW)', async () => {
    ...
    if (productsResponse.data && productsResponse.data[0]) {
      ...
      const configResponse = await testNetworkRequest(
        `${BASE_URL}/api/products/${productId}/configuration`
      )
  ...
  await runTest('TEST 7: Performance Metrics', async () => {
    ...
    const endpoints = ['/api/health', '/api/products', '/api/sizes', '/api/quantities']

  ...
  try {
    ...
    const quantityGroup = await prisma.quantityGroup.create({
      data: {
        ...
        quantities: {
          create: [
            { quantity: 100, price: 25.00, isActive: true },
            { quantity: 250, price: 50.00, isActive: true },
            { quantity: 500, price: 85.00, isActive: true },
            { quantity: 1000, price: 150.00, isActive: true },
            { quantity: 2500, price: 300.00, isActive: true }
          ]
        }
      },
      include: {
    ...
    console.log(`✅ Created quantity group: ${quantityGroup.name} with ${quantityGroup.quantities.length} quantities`);
    ...
    console.log(`✅ Created paper stock set: ${paperStockSet.name} with ${paperStockSet.PaperStockSetItem.length} paper options`);
    ...
    console.log(`📊 Quantities: ${product.productQuantityGroups[0]?.QuantityGroup?.quantities?.length || 0} options`);
    console.log(`📐 Sizes: ${product.productSizeGroups[0]?.SizeGroup?.sizes?.length || 0} options`);
    console.log(`📄 Paper Stocks: ${product.productPaperStockSets[0]?.PaperStockSet?.PaperStockSetItem?.length || 0} options`);

...
async function runOrderFlowTest(testRun) {
  ...
  try {
    ...
    try {
      ...
      results.steps.push({ step: 2, name: 'Product configuration loaded', success: false, error: error.message });
    }
    ...
    try {
      ...
      await page.evaluate(() => {
        ...
        const sizeSelectors = document.querySelectorAll('select[name*="size"], select[name*="Size"]');
        ...
        const paperSelectors = document.querySelectorAll('select[name*="paper"], select[name*="stock"]');
      ...
      results.steps.push({ step: 3, name: 'Configure product options', success: false, error: error.message });
    }
    ...
    try {
      ...
      const addToCartResult = await page.evaluate(() => {
        ...
        let addButton = buttons.find(btn => 
    ...
    try {
      ...
      const needsLogin = await page.evaluate(() => {
        return document.body.textContent.toLowerCase().includes('sign in') ||
      ...
      if (needsLogin) {
        ...
        const guestCheckout = await page.evaluate(() => {
          ...
          const guestBtn = guestButtons.find(btn =>
      ...
      await page.evaluate((customerInfo) => {
        ...
        fillField(['input[name="name"], input[name="firstName"], input[name="fullName"]'], customerInfo.name);
        fillField(['input[name="phone"], input[type="tel"]'], customerInfo.phone);
        fillField(['input[name="address"], input[name="street"], input[name="address1"]'], customerInfo.address.street);
        ...
        fillField(['input[name="zip"], input[name="zipCode"], input[name="postalCode"]'], customerInfo.address.zip);
      }, CUSTOMER_INFO);
      ...
      results.steps.push({ step: 7, name: 'Fill customer information', success: false, error: error.message });
    }
    ...
    try {
      ...
      const shippingSelected = await page.evaluate(() => {
        ...
        const shippingInput = shippingInputs.find(input =>
      ...
      results.steps.push({ step: 8, name: 'Select shipping method', success: shippingSelected.success });
    } catch (error) {
      log(`⚠️ Shipping selection error: ${error.message}`, 'warning');
      results.steps.push({ step: 8, name: 'Select shipping method', success: false, error: error.message });
    }
    ...
    try {
      ...
      const paymentSelected = await page.evaluate(() => {
        ...
        const cashElement = elements.find(el =>
        ...
        if (cashElement) {
          if (cashElement.tagName === 'INPUT') {
            ...
            const input = cashElement.querySelector('input') || 
                         document.querySelector(`input[value*="cash" i]`);
            if (input) {
              input.click();
              return { success: true, method: 'associated input', text: cashElement.textContent.trim() };
            }
            cashElement.click();
            return { success: true, method: 'element clicked', text: cashElement.textContent.trim() };
          }
        }
      ...
      if (paymentSelected.success) {
        log(`✅ Selected payment method: ${paymentSelected.text} (${paymentSelected.method})`, 'success');
      } else {
      ...
      results.steps.push({ step: 9, name: 'Select Test Cash Payment', success: paymentSelected.success });
    } catch (error) {
      log(`⚠️ Payment selection error: ${error.message}`, 'warning');
      results.steps.push({ step: 9, name: 'Select Test Cash Payment', success: false, error: error.message });
    }
    ...
    try {
      ...
      if (orderCompleted.success) {
        ...
        const orderInfo = await page.evaluate(() => {
          ...
          const orderNumberMatch = bodyText.match(/order\s*#?\s*(\d+)/i) || 
                                 bodyText.match(/order\s*number:?\s*([A-Z0-9-]+)/i) ||
                                 bodyText.match(/confirmation\s*#?\s*([A-Z0-9-]+)/i);
          
          const success = bodyText.toLowerCase().includes('order placed') ||
...
async function runCompleteTest() {
  ...
  console.log(`🕒 Test Duration: ${allResults[0]?.startTime} to ${allResults[allResults.length-1]?.endTime}`);

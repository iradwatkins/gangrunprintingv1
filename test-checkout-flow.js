...
async function testCheckout() {
  ...
  try {
    ...
    const testCashResult = await makeRequest('/api/checkout/create-test-order', 'POST', testOrderData);
    ...
    try {
      ...
      if (squareResult.data.success) {
        ...
        console.log('   ⚠️  Square checkout failed (expected if Square not configured):', squareResult.data.error);
      }
    } catch (error) {
      console.log('   ⚠️  Square checkout error (expected if Square not configured):', error.error || error);
    }

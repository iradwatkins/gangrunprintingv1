...
const REAL_DATA = {
  categories: [{ id: 'cat_banner', name: 'Banner', slug: 'banner' }],
  paperStocks: [{ id: 'cmg46sc60000f12ymdo48kpb0', name: '16pt C2S Cardstock' }],
...
async function investigateProductError() {
  ...
  try {
    ...
    if (currentUrl.includes('/auth/signin')) {
      ...
      console.log(
        'Email filled, but magic link auth requires email check. Skipping automated login.'
      )
    ...
    await page.selectOption(
      'select[name="turnaroundTimeGroupId"]',
      REAL_DATA.turnaroundGroups[0].id
    )
    ...
    try {
      ...
      const errors = await page.$$eval(
        '[role="alert"], .text-red-500, .text-red-600, .error',

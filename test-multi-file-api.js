  ...
  try {
    ...
    if (productWithImages && productWithImages.ProductImage.length > 0) {
      ...
      sortedImages.forEach((img, idx) => {
        console.log(`      ${idx + 1}. Order: ${img.sortOrder}, Primary: ${img.isPrimary}, Size: ${img.fileSize} bytes`);
      });
    ...
    if (productWithImages && productWithImages.ProductImage.length > 1) {
      ...
      // Test GET images API
      const getResponse = await fetch(`https://gangrunprinting.com/api/products/images?productId=${testProduct.id}`, {
        headers: {
          'Cookie': `${sessionCookie.name}=${sessionCookie.value}`
        }
      });
      ...
      if (productWithImages.ProductImage.length > 1) {
        ...
        const deleteResponse = await fetch(`https://gangrunprinting.com/api/products/images?imageId=${lastImage.id}`, {
          method: 'DELETE',
          headers: {
            'Cookie': `${sessionCookie.name}=${sessionCookie.value}`
          }
    ...
    if (finalProduct) {
      ...
      console.log(`✅ Primary image: ${finalProduct.ProductImage.some(img => img.isPrimary) ? 'Set' : 'Missing'}`);

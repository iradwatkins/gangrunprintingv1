...
function createTestImage() {
  if (!fs.existsSync(TEST_IMAGE_PATH)) {
    ...
    throw new Error(
      'Test image not found. Please ensure public/images/product-placeholder.jpg exists.'
    )
  }
...
class ImageUploadTester {
  ...
  async login() {
    ...
    try {
      await this.page.goto(`${BASE_URL}/admin/products`, {
  ...
  async scenario1_uploadToExistingProduct() {
    ...
    try {
      ...
      await this.page
        ...
        .catch(() => console.log('  ⚠️  Timeout waiting for page load'))
      ...
      console.log(
        `  ${uploadSuccess ? '✅' : '⚠️'} Upload indicators: ${uploadSuccess ? 'Found' : 'Not found'}`
      )
      ...
      await this.page.goto(`${BASE_URL}/products/ira-watkins`, {
        waitUntil: 'networkidle0',
        timeout: 30000,
      })
      ...
      console.log(
        `  ${imageDisplayed ? '✅' : '❌'} Product image ${imageDisplayed ? 'displayed' : 'NOT displayed'}`
      )
      ...
      this.results.scenario1.passed =
        uploadSuccess && imageDisplayed && this.consoleErrors.length === 0
  ...
  async scenario2_createNewProductWithImage() {
    ...
    try {
      ...
      await this.page
        .waitForSelector('input[name="name"], input#name, form', { timeout: 10000 })
        .catch(() => {
      ...
      if (submitButton) {
        ...
        const productCreated =
          currentUrl.includes('/admin/products') && !currentUrl.includes('/new')

        console.log(
          `  ${productCreated ? '✅' : '❌'} Product ${productCreated ? 'created' : 'creation unclear'}`
        )

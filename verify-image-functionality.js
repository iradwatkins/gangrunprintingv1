  ...
  try {
    ...
    if (productsWithImages.length > 0) {
      ...
      product.ProductImage.slice(0, 3).forEach((img, idx) => {
        console.log(`      ${idx + 1}. ${img.url} (Primary: ${img.isPrimary}, Sort: ${img.sortOrder})`);
      });
    }
    ...
    const uploadAPIExists = fs.existsSync('/root/websites/gangrunprinting/src/app/api/products/upload-image/route.ts');
    const imageAPIExists = fs.existsSync('/root/websites/gangrunprinting/src/app/api/products/images/route.ts');
    ...
    const componentExists = fs.existsSync('/root/websites/gangrunprinting/src/components/admin/product-image-upload.tsx');
    console.log(`   🖼️  Multi-file Upload Component: ${componentExists ? '✅ Exists' : '❌ Missing'}`);

    if (componentExists) {
      const componentContent = fs.readFileSync('/root/websites/gangrunprinting/src/components/admin/product-image-upload.tsx', 'utf8');

  ...
  try {
    ...
    const product = await prisma.product.create({
      data: {
        ...
        productionTime: 3,
        description:
          'High-quality business cards with flexible size options. This product demonstrates the Size Module working independently without any other modules.',
    ...
    console.log(
      '❌ Quantity Module:',
      finalProduct.productQuantityGroups.length > 0 ? 'ENABLED' : 'DISABLED (Correct!)'
    )

    console.log(
      '✅ Size Module:',
      finalProduct.productSizeGroups.length > 0 ? 'ENABLED' : 'DISABLED'
    )
    ...
    console.log(
      '❌ Paper Stock Module:',
      finalProduct.productPaperStockSets.length > 0 ? 'ENABLED' : 'DISABLED (Correct!)'
    )
    console.log(
      '❌ Add-On Module:',
      finalProduct.productAddOnSets.length > 0 ? 'ENABLED' : 'DISABLED (Correct!)'
    )
    console.log(
      '❌ Turnaround Module:',
      finalProduct.productTurnaroundTimeSets.length > 0 ? 'ENABLED' : 'DISABLED (Correct!)'
    )

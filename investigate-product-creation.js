...
async function investigateProductCreation() {
  ...
  try {
    ...
    await page.type('textarea[name="description"]', 'This is a test product created during error investigation using Puppeteer.', { delay: 30 });
    ...
    const errorElements = await page.$$('[class*="error"], [role="alert"], .text-red-500, .text-red-600');

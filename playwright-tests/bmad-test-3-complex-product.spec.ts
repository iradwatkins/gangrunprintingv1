...
test.describe('BMAD Test 3: Complex Product Configuration with All Options', () => {
  const complexProduct = {
    ...
    sku: 'BC-DELUXE-001',
    description:
      'The ultimate business card package featuring premium cardstock, multiple finishing options, rush production capabilities, and comprehensive customization. Perfect for executives and professionals who demand the highest quality presentation materials.',
    ...
    productOptions: [
      {
        ...
        values: [
          ...
          { value: 'gloss', label: 'Gloss Lamination', additionalPrice: 5.0, isDefault: false },
          {
            value: 'soft-touch',
      ...
      },
      {
        ...
        values: [
          ...
          { value: 'rounded', label: 'Rounded Corners', additionalPrice: 10.0, isDefault: false },
          { value: 'custom', label: 'Custom Die Cut', additionalPrice: 25.0, isDefault: false },
        ],
      },
      {
        ...
        required: false,
        values: [
          { value: 'spot-uv', label: 'Spot UV Coating', additionalPrice: 35.0, isDefault: false },
          { value: 'embossing', label: 'Embossing', additionalPrice: 45.0, isDefault: false },
          {
            value: 'foil-gold',
            label: 'Gold Foil Stamping',
            additionalPrice: 55.0,
            isDefault: false,
          },
          {
            value: 'foil-silver',
            label: 'Silver Foil Stamping',
      ...
      },
      {
        ...
        values: [
          {
            value: 'standard',
            label: 'Standard Box Packaging',
            additionalPrice: 0,
            isDefault: true,
          },
          { value: 'premium', label: 'Premium Gift Box', additionalPrice: 20.0, isDefault: false },
          { value: 'eco', label: 'Eco-Friendly Packaging', additionalPrice: 5.0, isDefault: false },
        ],
      },
    ],
  }
  ...
  test('Step 1: Create Complex Product with All Configurations', async ({ page }) => {
    ...
    const dropdownTests = [
      {
        selector: 'select[name="categoryId"], button:has-text("Select category")',
        option: 'Business Cards',
        name: 'Category',
      },
      {
        selector: 'select[name="selectedPaperStockSet"], button:has-text("Select paper stock set")',
        option: 'Standard Cardstock Set',
        name: 'Paper Stock Set',
      },
      {
        selector: 'select[name="selectedQuantityGroup"], button:has-text("Select quantity group")',
        option: 'Business Card Quantities',
        name: 'Quantity Group',
      },
      {
        selector: 'select[name="selectedSizeGroup"], button:has-text("Select size group")',
        option: 'Business Card Sizes',
        name: 'Size Group',
      },
      {
        selector: 'select[name="selectedAddOnSet"], button:has-text("Select add-on set")',
        option: 'Premium Business Card Add-ons',
        name: 'Add-on Set',
      },
      {
        selector:
          'select[name="selectedTurnaroundTimeSet"], button:has-text("Select turnaround time set")',
        option: 'Standard Production Times',
        name: 'Turnaround Time Set',
      },
    ]
    ...
    try {
      ...
      for (const tier of complexProduct.pricingTiers) {
        ...
        const addTierButton = page.locator(
          'button:has-text("Add Tier"), button:has-text("Add Pricing")'
        )
  ...
  test('Step 3: Add Product Options to Complex Product', async ({ page }) => {
    ...
    try {
      for (const option of complexProduct.productOptions) {
        ...
        const addOptionButton = page.locator(
          'button:has-text("Add Option"), button:has-text("Add Product Option")'
        )

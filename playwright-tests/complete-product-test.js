...
async function testImageUpload(page, imagePath, productName) {
  ...
  try {
    ...
    const uploadedImage = await page.locator('img[alt*="Product"], img[alt*="product"], .image-preview img, [data-testid="uploaded-image"]').first();
...
async function createProduct(page, product, index) {
  ...
  try {
    ...
    if (await categorySelect.count() > 0) {
      ...
      const categoryTrigger = await page.locator('[data-testid="category-select"] button, button:has-text("Select category")').first();
      if (await categoryTrigger.count() > 0) {
        await categoryTrigger.click();
        await page.waitForSelector('[role="option"]', { timeout: 5000 });
        await page.locator(`[role="option"]:has-text("${product.category}")`, { timeout: 5000 }).first().click();
    ...
    if (await firstPaperStock.count() > 0) {
      ...
      const defaultPaperRadio = await page.locator('input[type="radio"][name="defaultPaperStock"]').first();
    ...
    const submitButton = await page.locator('button:has-text("Save Product"), button[type="submit"]').first();
    ...
    if (response) {
      ...
      if (status === 200 || status === 201) {
        ...
        await page.waitForTimeout(2000);
        const successMessage = await page.locator('.toast-success, [role="alert"]:has-text("success")').first();
        ...
        log(`   ❌ Product creation failed (${status}): ${errorData.error || 'Unknown error'}`, colors.red);
...
async function runCompleteTest() {
  ...
  try {
    ...
    const productCount = await page.locator('[data-testid="product-row"], .product-item, tr:has(td)').count();
    ...
    log('==================================================', colors.blue);
    log(`✅ Admin Account Created: ${results.adminCreated}`, results.adminCreated ? colors.green : colors.red);
    log(`✅ Authentication Session: ${results.sessionCreated}`, results.sessionCreated ? colors.green : colors.red);
    log(`📦 Products Created: ${results.productsCreated}/${TEST_CONFIG.products.length}`,
         results.productsCreated === TEST_CONFIG.products.length ? colors.green : colors.yellow);
    log(`📸 Images Uploaded: ${results.imagesUploaded}/${TEST_CONFIG.products.length}`,

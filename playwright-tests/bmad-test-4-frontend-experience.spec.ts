...
test.describe('BMAD Test 4: Frontend Customer Experience Verification', () => {
  ...
  test('Step 1: Verify Homepage Product Display', async ({ page }) => {
    ...
    if ((await productCards.count()) > 0) {
      ...
      const productsLink = page.locator(
        'a[href*="products"], a:has-text("Products"), nav a:has-text("Shop")'
      )
      if ((await productsLink.count()) > 0) {
        await productsLink.first().click()
        await page.waitForLoadState('networkidle')

        const productsPageCards = page.locator(
          '[data-testid="product-card"], .product-card, [class*="product"]'
        )
      ...
      try {
        ...
        if (!is404) {
          ...
          const productElements = {
            ...
            'Add to Cart Button':
              'button:has-text("Add to Cart"), button:has-text("Order"), button[type="submit"]',
    ...
    for (const slug of testProductSlugs) {
      try {
        ...
        const configOptions = page.locator(
          'select, input[type="radio"], input[type="checkbox"], .quantity-selector'
        )
  ...
  test('Step 4: Test Add to Cart Functionality', async ({ page }) => {
    ...
    if (productToTest) {
      ...
      const cartCounters = page.locator(
        '.cart-count, [data-testid="cart-count"], .cart-items-count'
      )
      ...
      try {
        ...
        if (!is404) {
          ...
          const checkoutElements = {
            ...
            'Submit Button':
              'button[type="submit"], button:has-text("Place Order"), button:has-text("Submit")',
          }
  ...
  test('Step 6: Test API Endpoints from Frontend', async ({ page }) => {
    const apiEndpoints = [
      ...
      {
        endpoint: '/api/products/by-slug/premium-business-cards-test',
        name: 'Product by Slug API',
      },
  ...
  test('Step 7: Complete Customer Journey Simulation', async ({ page }) => {
    ...
    const productLinks = page.locator(
      'a[href*="/products/"], .product-card a, [data-testid="product-link"]'
    )
    ...
    const checkoutButtons = page.locator(
      'button:has-text("Checkout"), a[href*="checkout"], .checkout-button'
    )

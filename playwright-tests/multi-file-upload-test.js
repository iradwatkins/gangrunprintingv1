...
async function testMultiFileUpload() {
  ...
  try {
    ...
    // Test GET images
    const getResponse = await page.evaluate(async (productId, cookie) => {
      const response = await fetch(`/api/products/images?productId=${productId}`, {
        headers: { 'Cookie': cookie }
      });
      return { status: response.status, ok: response.ok };
    }, createdProduct?.id, `${sessionCookie.name}=${sessionCookie.value}`);
    ...
    if (finalProduct) {
      ...
      log(`✅ Primary image: ${finalProduct.ProductImage.some(img => img.isPrimary) ? 'Set' : 'Missing'}`,

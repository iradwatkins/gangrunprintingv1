...
async function createRealOrder(orderNumber) {
  ...
  try {
    ...
    await page.waitForSelector('[data-testid="quantity-selector"], select[name="quantity"]', {
      timeout: 10000,
    })
    await delay(1000)
    ...
    const quantitySelector = await page.$(
      '[data-testid="quantity-selector"], select[name="quantity"]'
    )
    if (quantitySelector) {
    ...
    if (fileInput) {
      ...
      await fileInput.uploadFile(
        '/Users/irawatkins/Documents/Git/gangrunprintingv1/public/logo.png'
      )
    ...
    await page.type(
      'input[name="zipCode"], input[name="zip"], input[placeholder*="ZIP"]',
      TEST_CUSTOMER.address.zipCode
    )
    ...
    const shippingOptions = await page.$$(
      'input[name="shipping"], input[type="radio"][name*="shipping"]'
    )
    ...
    const continueToPayment = await page.$(
      'button:has-text("Continue to Payment"), button:has-text("Continue"), button:has-text("Next")'
    )
...
async function runRealOrderTests() {
  ...
  log.info(
    `Delivery address: ${TEST_CUSTOMER.address.street}, ${TEST_CUSTOMER.address.city}, ${TEST_CUSTOMER.address.state} ${TEST_CUSTOMER.address.zipCode}`
  )
  ...
  console.log(
    `${colors.bright}${colors.cyan}🌐 Check admin panel at ${SITE_URL}/admin/orders${colors.reset}\n`
  )

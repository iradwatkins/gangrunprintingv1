...
;(async () => {
  ...
  try {
    ...
    const hasError310 = consoleMessages.some(
      (msg) => msg.text.includes('Error #310') || msg.text.includes('Minified React error #310')
    )

    // Check for product configuration elements
    const hasQuantitySelector =
      (await page.$(
        '[data-testid="quantity-selector"], select[name="quantity"], input[name="quantity"]'
      )) !== null
    const hasSizeSelector =
      (await page.$('[data-testid="size-selector"], select[name="size"], [class*="size"]')) !== null
    const hasPaperStockSelector =
      (await page.$(
        '[data-testid="paper-stock-selector"], select[name="paperStock"], [class*="paper"]'
      )) !== null

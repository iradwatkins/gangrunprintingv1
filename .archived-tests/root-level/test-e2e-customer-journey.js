  ...
  try {
        ...
        for (const selector of quantitySelectors) {
          ...
          if (input) {
            if (input.tagName === 'SELECT') {
              ...
              const closest =
                options.find((opt) => parseInt(opt.value) >= qty) || options[options.length - 1]
              if (closest) {
    ...
    try {
      ...
      await page.evaluate((data) => {
        ...
        fillField(
          ['input[name="street"], input[placeholder*="street" i], input[placeholder*="address" i]'],
          data.street
        )
        fillField(['input[name="city"], input[placeholder*="city" i]'], data.city)
        fillField(
          ['input[name="state"], select[name="state"], input[placeholder*="state" i]'],
          data.state
        )
        fillField(
          ['input[name="zip"], input[name="zipCode"], input[placeholder*="zip" i]'],
          data.zip
        )
        fillField(
    ...
    try {
      ...
      const orderPlaced = await page.evaluate(() => {
        ...
        const placeOrderBtn = buttons.find(
          (btn) =>
      ...
      if (orderPlaced) {
        ...
        const orderInfo = await page.evaluate(() => {
          ...
          const orderNumberMatch =
            text.match(/order\s*#?\s*(\d+)/i) || text.match(/order\s*number:?\s*([A-Z0-9-]+)/i)
          return {
    ...
    try {
      ...
      const orderStatus = await page.evaluate(() => {
        ...
        const processingEl = statusElements.find(
          (el) => el.textContent.includes('PROCESSING') || el.textContent.includes('Processing')
        )
      ...
      if (orderStatus.found) {
        log(
          customer,
          `✓ Order verified in account with status: ${orderStatus.statusText}`,
          'success'
        )
        results.steps.push({
          step: 'Verify order in account',
          success: true,
...
;(async () => {
  ...
  allResults.forEach((result, index) => {
    ...
    console.log(
      `   Completed Steps: ${result.steps.filter((s) => s.success).length}/${result.steps.length}`
    )

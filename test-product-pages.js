...
async function testProductPage(url, pageName) {
  ...
  try {
    ...
    const elements = {
      ...
      hasPriceDisplay: pageContent.includes('$') && (pageContent.includes('price') || pageContent.includes('Price')),
      hasAddToCart: pageContent.includes('Add to Cart') || pageContent.includes('add to cart'),
      hasProductTitle: await page.$('h1') !== null,
      hasConfigForm: await page.$('form') !== null || await page.$('[class*="config"]') !== null
    };
    ...
    const screenshotPath = path.join(__dirname, `test-${pageName.replace(/\s+/g, '-').toLowerCase()}.png`);
    ...
    console.log(`  Reload Count: ${reloadCount} ${hasReloadLoop ? '❌ LOOP DETECTED' : '✅ STABLE'}`);
    ...
    // Overall assessment
    const isSuccess = !hasReactError310 &&
                     !hasReloadLoop &&
    ...
    try {
      const screenshotPath = path.join(__dirname, `test-${pageName.replace(/\s+/g, '-').toLowerCase()}-error.png`);

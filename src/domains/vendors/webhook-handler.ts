...
export const VENDOR_STATUS_MAPPINGS: Record<string, VendorStatusMapping[]> = {
  ...
  default: [
    ...
    { vendorStatus: 'prepress', internalStatus: OrderStatus.PREPRESS, event: 'vendor_accepted' },
    {
      ...
      internalStatus: OrderStatus.PRODUCTION,
    ...
    // Hold statuses
    {
      vendorStatus: 'hold_bad_files',
      internalStatus: OrderStatus.ON_HOLD_BAD_FILES,
      event: 'bad_files_detected',
    },
    {
      vendorStatus: 'hold_bad_images',
      internalStatus: OrderStatus.ON_HOLD_BAD_IMAGES,
      event: 'bad_images_detected',
    },
    {
      vendorStatus: 'hold_missing_files',
      internalStatus: OrderStatus.ON_HOLD_FILE_MISSING,
      event: 'file_missing',
    },
    {
      vendorStatus: 'hold_text_edges',
      internalStatus: OrderStatus.ON_HOLD_TEXT_CLOSE_TO_EDGES,
  ...
  async processWebhook(payload: VendorWebhookPayload): Promise<{
    ...
    try {
      ...
      const mapping = mappings.find(
        (m) => m.vendorStatus.toLowerCase() === payload.status.toLowerCase()
      )

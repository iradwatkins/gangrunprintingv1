...
async function getTemplates() {
  // Don't load base64 images here - they're 4-10MB each and slow down page load
  // Images will be loaded client-side progressively
  const templates = await prisma.promptTemplate.findMany({
    include: {
      testImages: {
        select: {
          id: true, // Only get the ID, not the base64 imageUrl
        },
        orderBy: [
          { isWinner: 'desc' },
          { quality: 'desc' },
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Browse all prompts by category - use them as templates or continue editing
          </p>
        </div>
      </div>

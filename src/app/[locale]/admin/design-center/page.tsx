import { Link } from '@/lib/i18n/navigation'
import { Button } from '@/components/ui/button'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Sparkles, FileText, Beaker, Settings } from 'lucide-react'
import { prisma } from '@/lib/prisma'
import { CreatePromptForm } from './create-prompt-form'

async function getPromptStats() {
  const [totalPrompts, totalTemplates, recentTests] = await Promise.all([
    prisma.promptTemplate.count({
      where: { isTemplate: false },
    }),
    prisma.promptTemplate.count({
      where: { isTemplate: true },
    }),
    prisma.promptTestImage.count({
      where: {
        createdAt: {
          gte: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000), // Last 7 days
        },
      },
    }),
  ])

  return { totalPrompts, totalTemplates, recentTests }
}

export default async function PromptsPage() {
  const stats = await getPromptStats()

  return (
    <div className="flex-1 space-y-6 p-8">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight flex items-center gap-2">
            <Sparkles className="h-8 w-8 text-primary" />
            Design Center
          </h1>
          <p className="text-muted-foreground mt-1">
            Create stunning product images with AI - build custom prompts or browse templates
          </p>
        </div>
        <div className="flex gap-2">
          <Link href="/admin/design-center/settings">
            <Button variant="outline" size="icon" title="API Settings">
              <Settings className="h-4 w-4" />
            </Button>
          </Link>
          <Link href="/admin/design-center/templates">
            <Button variant="outline">
              <FileText className="h-4 w-4 mr-2" />
              Prompt Library
            </Button>
          </Link>
        </div>
      </div>

      {/* Stats Cards */}
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Your Prompts</CardTitle>
            <Sparkles className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{stats.totalPrompts}</div>
            <p className="text-xs text-muted-foreground">Active saved prompts</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Templates</CardTitle>
            <FileText className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{stats.totalTemplates}</div>
            <p className="text-xs text-muted-foreground">Available templates</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Recent Tests</CardTitle>
            <Beaker className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{stats.recentTests}</div>
            <p className="text-xs text-muted-foreground">Last 7 days</p>
          </CardContent>
        </Card>
      </div>

      {/* Create Prompt Form */}
      <CreatePromptForm />
    </div>
  )
}

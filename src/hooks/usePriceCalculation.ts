...
export function usePriceCalculation({ configData, getQuantityValue }: UsePriceCalculationProps) {
  ...
  const calculateBasePrice = useCallback((config: SimpleProductConfiguration): number => {
  ...
  const calculateSpecialAddonCosts = useCallback((config: SimpleProductConfiguration, quantity: number): number => {
  ...
  const calculateFinalPrice = useCallback((config: SimpleProductConfiguration): number => {
    if (!configData) return 0
  ...
  const getPriceBreakdown = useCallback((config: SimpleProductConfiguration) => {
  ...
  }, [configData, getQuantityValue, calculateBasePrice, calculateFinalPrice, calculateSpecialAddonCosts])

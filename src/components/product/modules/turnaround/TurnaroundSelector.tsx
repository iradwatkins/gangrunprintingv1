...
export default function TurnaroundTimeSelector({
  ...
  const isOptionRestricted = (turnaround: TurnaroundTime): boolean => {
    // Only check restricted coatings list - requiresNoCoating is just informational
    if (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      {/* Show "Requires no coating" as informational note (not a restriction) */}
                      {turnaround.requiresNoCoating && !isRestricted && (
                        <div className="text-xs font-semibold text-orange-600 mt-1">
                          *ORDER WILL BE PRINTED W/ NO COATING
                        </div>
                      )}
                      {/* Show restriction message for coating restrictions */}
                      {isRestricted && (

    ...
    <div className={cn('border rounded-lg overflow-hidden', className)}>
      ...
      {!readOnly && (
        <div className="flex items-center justify-between p-2 bg-gray-50 border-t text-xs text-gray-500">
          <span>
            Use Tab for indentation, Ctrl+A to select all
          </span>
          <span>
...
export function SyntaxHighlighter({
  ...
  return (
    <pre
      className={cn(
        'bg-gray-50 p-4 rounded-lg overflow-x-auto text-sm font-mono',
        className
      )}
...
function formatCode(code: string, language: string): string {
  ...
  if (language === 'css') {
    ...
    // Keywords
    const keywords = ['function', 'const', 'let', 'var', 'if', 'else', 'return', 'for', 'while', 'class', 'extends'];

  ...
  return (
    <Card className="border-2 border-gray-200">
      <CardHeader>
        ...
        <CardDescription>
          Configure optional features like add-ons and turnaround times.
      ...
      <CardContent className="space-y-6">
        <div className="space-y-2">
          ...
          {formData.selectedAddOnSet && formData.selectedAddOnSet !== "none" && (
            <p className="text-xs text-purple-600">✓ Selected: {addOnSets.find(s => s.id === formData.selectedAddOnSet)?.name}</p>
          )}
          {(!formData.selectedAddOnSet || formData.selectedAddOnSet === "none") && (
        ...
        <div className="space-y-2">
          ...
          {formData.selectedTurnaroundTimeSet && formData.selectedTurnaroundTimeSet !== "none" && (
            <p className="text-xs text-purple-600">✓ Selected: {turnaroundTimeSets.find(s => s.id === formData.selectedTurnaroundTimeSet)?.name}</p>
          )}
          {(!formData.selectedTurnaroundTimeSet || formData.selectedTurnaroundTimeSet === "none") && (
            <p className="text-xs text-gray-500 flex items-center gap-1">
              <Info className="h-3 w-3" />
        ...
        <div className="mt-6 p-4 bg-purple-50 rounded-lg border border-purple-200">
          ...
          <div className="space-y-1 text-xs text-purple-600">
            <div className="flex items-center gap-2">
              ...
              Add-ons: {formData.selectedAddOnSet && formData.selectedAddOnSet !== "none"
            ...
            <div className="flex items-center gap-2">
              ...
              Turnaround Times: {formData.selectedTurnaroundTimeSet && formData.selectedTurnaroundTimeSet !== "none"

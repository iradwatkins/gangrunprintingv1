...
export interface AddOnConfiguration {
  ...
  design?: {
    selected: boolean;
    serviceType: 'upload_artwork' | 'standard_custom' | 'rush_custom' | 'minor_changes' | 'major_changes';
    sides: 'one' | 'two';
  };
...
export class PricingEngine {
  ...
  calculatePrice(config: ProductConfiguration): PriceCalculation {
    ...
    if (config.isBroker && config.brokerDiscounts) {
      ...
      if (categoryDiscount) {
        ...
        result.adjustedBasePrice = result.basePaperPrintPrice * (1 - result.brokerDiscountPercentage / 100);
    ...
    // Apply "Our Tagline" discount (hidden for brokers with discount)
      ...
      const taglineDiscountAmount = result.basePaperPrintPrice * (result.taglineDiscountPercentage / 100);
    ...
    result.priceAfterTurnaround = result.priceAfterBasePercentageModifiers * 
      (1 + result.turnaroundMarkupPercentage / 100);
    result.breakdown.turnaroundMarkup = result.priceAfterTurnaround - result.priceAfterBasePercentageModifiers;
    ...
    // Final Calculation
    result.calculatedProductSubtotalBeforeShippingTax = result.priceAfterTurnaround + result.totalAddonCost;
  ...
  private calculateAddonCosts(config: ProductConfiguration): Array<{name: string; cost: number; calculationDetails: string}> {
    ...
    if (addons.digitalProof?.selected) {
      costs.push({
        name: 'Digital Proof',
        cost: addons.digitalProof.price || 5.00,
    ...
    if (addons.perforation?.selected) {
      const setupFee = addons.perforation.setupFee || 20.00;
    ...
    if (addons.scoreOnly?.selected) {
      const setupFee = addons.scoreOnly.setupFee || 17.00;
    ...
    if (addons.folding?.selected) {
      ...
      const note = addons.folding.paperType === 'card_stock' ? ' (includes mandatory basic score)' : '';
      costs.push({
        name: 'Folding',
        cost: cost,
    ...
    if (addons.shrinkWrapping?.selected) {
      ...
      const pricePerBundle = addons.shrinkWrapping.pricePerBundle || 0.30;
      const bundles = Math.ceil(config.quantity / itemsPerBundle);
      const cost = bundles * pricePerBundle;
      costs.push({
    ...
    if (addons.postalDelivery?.selected) {
      ...
      const pricePerBox = addons.postalDelivery.pricePerBox || 30.00;
    ...
    if (addons.eddmProcess?.selected) {
      const setupFee = addons.eddmProcess.setupFee || 50.00;
    ...
    if (addons.holeDrilling?.selected) {
      const setupFee = addons.holeDrilling.setupFee || 20.00;
      let perPieceCost = 0;
      let description = '';

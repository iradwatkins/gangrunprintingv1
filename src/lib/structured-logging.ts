...
export type LogContext = 'api' | 'auth' | 'business' | 'system' | 'user' | 'payment' | 'order' | 'product';
...
class StructuredLogger {
  ...
  error(message: string, context: LogContext = 'system', error?: Error, metadata?: Record<string, any>): void {
  ...
  fatal(message: string, context: LogContext = 'system', error?: Error, metadata?: Record<string, any>): void {
  ...
  logBusinessEvent(
    ...
    const entry = this.createLogEntry('info', event, 'business', metadata, undefined, undefined, businessData);
    this.outputLog(entry);
    this.sendToMonitoring(entry);
  }
  ...
  logPaymentEvent(
    event: string,
    paymentData: { orderId: string; amount: number; currency: string; method?: string; status?: string },
    metadata?: Record<string, any>
  ): void {
...
export const logApiResponse = (method: string, endpoint: string, statusCode: number, duration: number) => {
...
export const logUserActivity = (activity: string, userId: string, metadata?: Record<string, any>) => {
...
export const useLogging = () => {
  ...
  const setUserContext = (userId: string, sessionId?: string) => logger.setUserContext(userId, sessionId);

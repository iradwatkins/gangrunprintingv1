      ...
      <body style={{ fontFamily: 'Arial, sans-serif', maxWidth: '600px', margin: '0 auto', padding: '20px' }}>
        ...
        <div style={{ marginBottom: '24px' }}>
          <p style={{ fontSize: '16px', lineHeight: '1.6' }}>
            Thank you for choosing GangRun Printing! We're excited to bring your project to life.
          </p>
          <p style={{ fontSize: '16px', lineHeight: '1.6' }}>
            Your order #{orderNumber} for {formatPrice(orderTotal)} is now in production. We'll notify you as soon as it ships!
          </p>
        </div>
        ...
        <div style={{ background: '#f9fafb', padding: '20px', borderRadius: '8px', marginBottom: '24px' }}>
          ...
          <div style={{ fontSize: '14px', lineHeight: '1.8' }}>
            <p style={{ margin: '0 0 8px' }}><strong>1. Production:</strong> Your order is being printed with care</p>
            <p style={{ margin: '0 0 8px' }}><strong>2. Quality Check:</strong> We'll inspect every detail</p>
            <p style={{ margin: '0 0 8px' }}><strong>3. Shipping:</strong> Carefully packaged and shipped to you</p>
            <p style={{ margin: 0 }}><strong>4. Delivery:</strong> Track your shipment in real-time</p>
          </div>
        </div>
        ...
        {recommendedProducts && recommendedProducts.length > 0 && (
          <div style={{ background: '#eff6ff', padding: '20px', borderRadius: '8px', marginBottom: '24px' }}>
            ...
            {recommendedProducts.map((product, index) => (
              <div key={index} style={{ marginBottom: '12px', paddingBottom: '12px', borderBottom: index < recommendedProducts.length - 1 ? '1px solid #dbeafe' : 'none' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <p style={{ margin: '0 0 4px', fontWeight: 'bold' }}>{product.name}</p>
                    <p style={{ margin: 0, fontSize: '14px', color: '#737373' }}>Starting at {formatPrice(product.price)}</p>
                  </div>
                  <a
                    href={product.url}
                    style={{

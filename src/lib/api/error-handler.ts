...
export class ApiError extends Error {
  ...
  static database(message: string, cause?: Error): ApiError {
    return new ApiError(ApiErrorType.DATABASE, 'Database operation failed', 500, { original: message }, cause);
  }

  static externalService(service: string, message: string, cause?: Error): ApiError {
    return new ApiError(ApiErrorType.EXTERNAL_SERVICE, `${service} service error`, 503, { original: message }, cause);
  }
  ...
  static internal(message: string, cause?: Error): ApiError {
    return new ApiError(ApiErrorType.INTERNAL, 'Internal server error', 500, { original: message }, cause);
  }
}
  ...
  return async (request: NextRequest, routeParams?: { params: any }): Promise<NextResponse> => {
    ...
    try {
      ...
      if (options?.rateLimit) {
        const { checkRateLimit, getRateLimitIdentifier, formatRateLimitError } = 

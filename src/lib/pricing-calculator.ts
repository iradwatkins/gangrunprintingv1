...
export class PricingCalculator {
  ...
  static calculate(inputs: PricingInputs): PricingBreakdown {
    ...
    const sidesMultiplier = inputs.sides === 'double' ? 
      (1 + (inputs.paperStock.secondSideMarkup || 100) / 100) : 1
    
    const basePaperPrintPrice = 
      inputs.paperStock.pricePerSquareInch * 
      sizeInSquareInches * 
      inputs.quantity * 
      sidesMultiplier
    ...
    const brokerDiscountAmount = inputs.brokerDiscount ? 
      basePaperPrintPrice * (inputs.brokerDiscount / 100) : 0
    ...
    const exactSizeAddOn = inputs.addOns?.find(a => 
      a.name.toLowerCase().includes('exact size')
    )
    ...
    const turnaroundMarkup = inputs.turnaroundMarkup ? 
      priceAfterExactSize * (inputs.turnaroundMarkup / 100) : 0
  ...
  static formatBreakdown(breakdown: PricingBreakdown): string[] {
    ...
    lines.push(`  (${breakdown.breakdown.sizeInSquareInches} sq in × $${breakdown.breakdown.paperCost}/sq in × ${breakdown.breakdown.quantityUsed} qty × ${breakdown.breakdown.sidesMultiplier}x sides)`)

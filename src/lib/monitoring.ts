...
export class BusinessMetricsTracker {
  ...
  trackOrderCreated(orderId: string, orderValue: number, itemCount: number, paymentMethod: string) {
    ...
    addBreadcrumb(
      'Order created',
      'business',
      { orderId, orderValue, itemCount, paymentMethod }
    );
  }
...
// Resource monitoring
export function trackResourceLoad(type: 'image' | 'script' | 'style' | 'font', url: string, loadTime: number) {

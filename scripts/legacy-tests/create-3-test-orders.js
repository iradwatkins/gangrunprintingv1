...
async function createTestOrder(testData, orderNumber) {
  ...
  try {
    const airport = await prisma.airport.findFirst({
      where: { code: testData.airport, isActive: true, carrier: 'SOUTHWEST_CARGO' },
    })
    ...
    const order = await prisma.order.create({
      data: {
        ...
        subtotal: testData.quantity * 0.1,
        tax: testData.quantity * 0.1 * 0.0825,
        shipping: 0,
        total: testData.quantity * 0.1 * 1.0825,
        shippingAddress: {
          street: '123 Test St',
          city: airport.city,
          state: airport.state,
          ...
          create: [
            {
              ...
              productSku: `TEST-SKU-${testData.airport}`,
        ...
        OrderFile: {
          create: [
            {
              id: createId(),
              filename: `test-image-${testData.airport}.png`,
              fileUrl: `https://gangrun-test.s3.amazonaws.com/test-${testData.airport}.png`,
              thumbnailUrl: `https://gangrun-test.s3.amazonaws.com/thumbnails/test-${testData.airport}.png`,
              fileSize: 1024,
              mimeType: 'image/png',
    ...
    console.log(
      `  📸 Files: ${order.OrderFile.length} (Thumbnail: ${order.OrderFile[0]?.thumbnailUrl ? 'YES' : 'NO'})`
    )

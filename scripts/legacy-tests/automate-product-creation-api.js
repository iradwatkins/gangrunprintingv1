...
async function createProductViaAPI(page, product, index) {
  ...
  try {
    ...
    if (categoriesResponse.category) {
      ...
      console.log(
        `  ⚠️  Category ${product.category} not found, available categories:`,
      ...
      console.log(
        `  - Using fallback category: ${categoriesResponse.allCategories[0]?.name} (${categoryId})`
      )
    }
    ...
    if (!result.ok) {
      const errorMsg =
        result.data?.error || result.data?.rawResponse || result.error || 'Unknown error'

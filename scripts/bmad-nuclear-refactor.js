...
class NuclearRefactor {
  ...
  parseApiRoute(content) {
    ...
    const utilRegex =
      /(?:const|function) (?!GET|POST|PUT|PATCH|DELETE)\w+[\s\S]*?(?=\n(?:const|function|export)|$)/g
...
export const DELETE = handleDelete;`
  ...
  printReport() {
    ...
    this.refactoredFiles.forEach((file) => {
      ...
      console.log(
        `   ${path.basename(file.path)}: ${file.lines} → ~${this.targetLineCount} lines (${reduction}% reduction)`
      )
    })
    ...
    console.log(
      `   Success rate: ${Math.round((this.refactoredFiles.length / (this.refactoredFiles.length + this.errors.length)) * 100)}%`
    )
  }
}

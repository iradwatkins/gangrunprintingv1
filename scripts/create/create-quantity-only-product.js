...
async function createQuantityOnlyProduct() {
  ...
  try {
    ...
    // Product description
    const description =
      'High-quality business cards available in various quantities. This product demonstrates the quantity module working independently.'
    ...
    const categoryExists = await page.evaluate(() => {
      ...
      if (select) {
        const option = Array.from(select.options).find(
          (opt) => opt.text.toLowerCase().includes('business') || opt.value.includes('business')
        )
        if (option) {
    ...
    if (categoryExists) {
      ...
      await page.select(
        categorySelector,
        await page.$eval(categorySelector + ' option:nth-child(2)', (el) => el.value)
      )
    }
    ...
    const moduleSelectors = {
      quantity:
        'input[name*="quantity" i][type="checkbox"], input[id*="quantity" i][type="checkbox"], label[for*="quantity" i] input[type="checkbox"]',
      size: 'input[name*="size" i][type="checkbox"], input[id*="size" i][type="checkbox"], label[for*="size" i] input[type="checkbox"]',
      paper:
        'input[name*="paper" i][type="checkbox"], input[id*="paper" i][type="checkbox"], label[for*="paper" i] input[type="checkbox"]',
      addon:
        'input[name*="addon" i][type="checkbox"], input[id*="addon" i][type="checkbox"], label[for*="addon" i] input[type="checkbox"]',
      turnaround:
        'input[name*="turnaround" i][type="checkbox"], input[id*="turnaround" i][type="checkbox"], label[for*="turnaround" i] input[type="checkbox"]',
    }
    ...
    console.log(
      `   ✅ Quantity Module: ${quantityEnabled ? 'Enabled' : 'Already enabled or not found'}`
    )
    ...
    if (quantityGroupExists) {
      ...
      await page.select(
        quantityGroupSelector,
        await page.$eval(quantityGroupSelector + ' option:nth-child(2)', (el) => el.value)
      )
...
createQuantityOnlyProduct()
  .then((result) => {
    if (result.success) {
      ...
      console.log(
        `Check it out at: https://gangrunprinting.com/products/${result.productId || 'new'}`
      )
    } else {

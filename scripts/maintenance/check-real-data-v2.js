...
async function checkRealData() {
  try {
    ...
    console.log('\n📐 SIZES:')
    sizes.forEach((s) =>
      console.log(`  - ${s.name}: ${s.width}x${s.height} ${s.unit} (ID: ${s.id})`)
    )
    ...
    paperStocks.forEach((ps) =>
      console.log(`  - ${ps.name} - ${ps.weight}${ps.weightUnit} ${ps.finish} (ID: ${ps.id})`)
    )
    ...
    products.forEach((p) =>
      console.log(`  - ${p.name} (${p.sku}) - Category: ${p.productCategory?.name}`)
    )

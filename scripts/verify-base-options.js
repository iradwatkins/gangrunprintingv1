...
async function verifyBaseOptions() {
  ...
  try {
    ...
    if (allMatch) {
      ...
      log(`\n📊 Totals:`, 'blue')
      log(`Paper Stocks: ${paperStockNames.length}/7`, paperStockNames.length === 7 ? 'green' : 'red')
    ...
    if (paperStocks.length > 0) {
      ...
      if (firstStock.paperStockCoatings && firstStock.paperStockCoatings.length > 0) {
        log(`✅ Paper stocks have coating relationships (${firstStock.paperStockCoatings.length} coatings)`, 'green')
      } else {
        log(`⚠️  Paper stocks missing coating relationships`, 'yellow')
      }

      if (firstStock.paperStockSides && firstStock.paperStockSides.length > 0) {
        log(`✅ Paper stocks have sides relationships (${firstStock.paperStockSides.length} sides)`, 'green')
      } else {
